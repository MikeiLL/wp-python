...
#        dur = track.duration
        print  "No Length Markers", markers[-1].start, " + ", markers[-1].duration, " - ", markers[0].start
        dur = markers[-1].start + markers[-1].duration - markers[0].start
    print "is_valid return", inter, "+ 2 *", transition, " < ", dur
...
def move_cursor(track, duration, cursor, buf=MIN_MARKERS):
    dur = 0
    print "Resampled:", track.filename, track.resampled
